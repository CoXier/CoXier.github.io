...
Publisher 将数据发送给 Subscriber，整个数据流处理遵循协议：

**onSubscribe onNext\* (onError | onComplete)?**

个人理解：

- onSubscribe 触发事件流、数据流


- Publisher 调用 0 次或多次 Subscriber#onNext 方法发射事件、数据


- Publisher 调用 0 次或者 1 次 Subscriber#onError/onComplete，结束事件流。一个好的事件流、数据流应该有一次 onError 或者 onComplete 的。

Reactive Stream 常见的图示：

![img](https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/legend.png)

# 一、Reactive Stream in RxJava2

在 RxJava 2.x 中，reactive class 有 Flowable、Observable、Single、Maybe、Completable，他们类似上面提到的 Publisher，起到发送数据的作用，其中 Flowable 实现了 Publisher 的接口，具备了 Backpressure，Observable 等其他的 reactive class 从 Flowable 衍生而来，不具备 Backpressure。
...
RxJava 的方法数多的原因之一是其拥有大量的操作符，RxJava 中所有的操作都是通过操作符进行的， 虽然 RxJava 中每个操作符都有与之对应的 Observable ，但是只有一类操作符是用来作为源头发射数据或者事件的，这类操作符是创建操作符，常见的创建操作符如下，[文档](https://github.com/ReactiveX/RxJava/wiki/Creating-Observables)

- **just**：接收1~10个参数，创建发射一个或者多个对象的 Observable，此方法槽点之一是：作者手写了 1 个参数 ~ 10 个参数的方法，just(T item1.....T item10)
...
使用 fromArray 小试牛刀，上游按照顺序发送两个字符串 "jianxin" 和 "fucheng"，下游收到字符串后打印出来，当上游发送完后，下游打印 "onComplete"，也就是遵循 onSubscribe onNext*2  onComplete 的协议。

![img](https://images-cdn.shimo.im/bu8A9sjv6S0gdaef/carbon_1_.png)

上面的代码可以分成两个部分来看：

* Observable.fromArray：创建一个 Observable，Observable.fromArray 操作符对应 ObservableFromArray，**在 RxJava 中每个操作符都是对应一个 Observable**。
* subscribe(xx)：订阅数据，数据开始发射。
...
![](https://images-cdn.shimo.im/9koaIM3FhacmZJ1w/carbon_2_1_.png)
...
RxJava 有很多过滤操作符，但是出发点都是过滤出 downstream 需要的数据。例如上面的代码，downstream 只想获取以字母 `j` 开头的 name。
...
RxJava 能转换上游发出的数据，经过变换后交给下游处理。下面有个例子，上游发送了 1 ， 2， 3 三个数据，经过 [map](http://reactivex.io/documentation/operators/map.html) 操作符处理后，下游接收到的是 20，40 ，60。
...
上面已经介绍了 map 的使用，下面再来看看如何使用 flatMap 也将上游发送的数据每个*20。

![](https://images-cdn.shimo.im/RKRAjAw4YW0F0qrG/carbon_2_.png!thumbnail)

大多数情况下，map 和 flatMap 都能实现一样的功能，**但是 flatMap 可以用来做一些和异步有关的事情**，但可能 map 无法做到。比如我们同时获取了一系列用户的 userId，然后希望通过这些 userId 分别请求每个用户的详细信息 UserInfo，每请求到一个 UserInfo 就更新相应的 UI，显然，请求用户详细信息的操作属于网络异步操作，所以应该不必等到所有的请求完成才更新 UI。
...
![](https://images-cdn.shimo.im/wzXubtHNoLEn8dJI/carbon_3_1_1_.png!thumbnail)

上面的代码省略了 UI 线程更新的具体逻辑，UI线程更新还是需要借助 Handler 来更新，一次 sendMessage 和 handlerMsg，其实代码是很**分散**的。如果换用 RxJava + Retrofit + RxJava2CallAdapter 整个流程就会收敛到一条线上。 [zip](http://reactivex.io/documentation/operators/zip.html) 操作符通过一个函数将多个 Observable 发出的数据结合为一个新的数据。

![](https://images-cdn.shimo.im/2Qpi48BzyYQt83Wi/carbon_2_1_.png)
...
![](https://images-cdn.shimo.im/29s9mxFmVKAZB0IO/carbon_1_.png!thumbnail)

在 RxJava 中将发送数据的一方叫做 **upstream**，接收数据的一方叫 **downstream**。如：

![img](https://images-cdn.shimo.im/SV3uIdMoz0MZzRlC/%E4%B8%8B%E8%BD%BD.png)
...
![](https://images-cdn.shimo.im/s96nHaUKflw1qNyw/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7_2018_09_01_%E4%B8%8A%E5%8D%8811.07.52.png)



* 在 ObservableObserveOn#subscribeActual 会调用 upstream ObservableSubcribeOn#subscribe
* 在 ObservableSubscribeOn 中为了实现上游 ObservableFromArray 的 subscribe 工作在另一个线程，使用了 Scheduler 和 Wroker 来调度 SubscribeTask，进而调用 ScheduledExecutorService 的 submit
* ObservableSubscribeOn#SubscribeTask 中调用 upstream ObservableFromArray 的 subscribe，之后事件源头开始发送数据

# 3.2 接收事件

每个 Observable 都有与之对应的 Observer，对应关系见下表：
...
![](https://images-cdn.shimo.im/E72JoRxSLQ8Kzb1y/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7_2018_09_01_%E4%B8%8B%E5%8D%885.04.25.png)

* 3.1 中讲到 ObservableFromArray 开始发射数据，ObservableFromArray 的 downstream 是 ObservableSubscriberOn，所以第一个接收到事件源发出数据的是 ObservableSubscriberOn 对应的 SubscribeOnObserver。通过 SubscribeOnObserver#onNext 将数据继续向 downstream 发送。
* ObservableSubscriberOn 的 downstream 是 ObservableObserveOn，ObservableObserveOn 的 observer 是 ObserveOnObserver。ObserveOnObserver 接收到数据后，先将数据入队。*
* ObserveOnObserver 为了向 downstream 发送数据时切换线程，用到了 worker，ObserveOnObserver 实现了 Runnable，在 ObserveOnObserver 执行 run 时，将队列的数据出队，然后向 downstream 发送。
